...
class JoernTool(AbstractTool):
    ...
    def run(self, target: str, scan_input: ScanInput) -> ScanResult:
        ...
        try:
            script = f'importCode(inputPath="{target}", projectName="scan"); println(cpg.finding.toJsonPretty); exit;'
            proc = subprocess.run(
            ...
            return ScanResult(
                tool=self.name, target=target, duration=0.0, status=ScanStatus.FAILED, error="Binary not found: joern"
            )
        finally:
