        ...
        try:
            ...
            for item in data.get("vulnerable", []):
                for vuln in item.get("Vulnerabilities", []):
                    ...
                    sev = Severity.CRITICAL if float(sev_str) >= 9 else \
                          Severity.HIGH if float(sev_str) >= 7 else \
                          Severity.MEDIUM if float(sev_str) >= 4 else Severity.LOW
    ...
    def run(self, target: str, scan_input: ScanInput) -> ScanResult:
        ...
        try:
            ...
            return ScanResult(tool=self.name, target=target, duration=0.0,
                              status=ScanStatus.FAILED, error="Binary not found: nancy")
