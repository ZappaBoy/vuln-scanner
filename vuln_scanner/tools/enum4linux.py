...
class Enum4linuxTool(AbstractTool):
    ...
    def asset_predicate(self, asset: Asset) -> bool:
        ...
        return port in self._SMB_PORTS or "smb" in service or "netbios" in service or "microsoft-ds" in service
        ...
        if ppol:
            ...
            findings.append(
                Finding(
                    ...
                    severity=sev,
                    description=(
                        f"Password policy: minimum length={min_len}, lockout threshold={lockout} (0 means no lockout)."
                    ),
                    tool=self.name,
                    target=target,
                    raw=ppol,
