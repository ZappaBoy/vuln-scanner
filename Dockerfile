...
# KICS ships its queries/libraries as on-disk assets, so it must be built from
# source to get both the binary and the matching assets/ directory (the release
# install.sh provides only the binary, which then fails to load any queries).
RUN --mount=type=cache,target=/go/pkg/mod \
    --mount=type=cache,target=/root/.cache/go-build \
...
# ── Copy pre-built binaries from builder stages ───────────────────────────────
...
# ── Base system ───────────────────────────────────────────────────────────────
# go and rust removed: their tools are provided by the builder stages above.
# Cache mount keeps downloaded packages out of image layers and reuses them
# across rebuilds.
RUN --mount=type=cache,target=/var/cache/pacman/pkg,sharing=locked \
    pacman -Syu --noconfirm && \
    pacman -S --noconfirm --needed \
    jdk-openjdk git base-devel sudo python python-pip unzip curl \
    nodejs npm ruby php openscap dotnet-sdk && \
...
# yay -Syu removed: pacman -Syu in the previous layer already upgraded all
# official packages, making a second upgrade pass redundant.
# dotnet-sdk removed: already installed via pacman above.
# Cache mounts keep the pacman pkg cache and the yay build cache out of image
# layers while reusing them between builds.
...
# ── pip packages ─────────────────────────────────────────────────────────────
# Every pip-installable tool goes through this single loop — plain PyPI names,
# plus git+ URLs for tools whose PyPI package is stale (knockpy) or absent
# (CRLFsuite, VHostScan). Each is installed individually so a native-dep build
# failure is non-fatal. requests_pkcs12 is a git-dumper runtime dep.
# The app's own dependencies are installed later via `uv sync` and are unaffected.
# Cache mount keeps the pip wheel cache out of image layers while reusing it
# between builds; --no-cache-dir is therefore not needed.
RUN --mount=type=cache,target=/root/.cache/pip \
    ...
    subdominator gato-x codechecker theharvester autosubtakeover \
...
# ── npm global tools ─────────────────────────────────────────────────────────
# Cache mount keeps the npm cache out of image layers while reusing it
# between builds.
RUN --mount=type=cache,target=/root/.npm \
    npm install -g eslint yarn --no-audit --no-fund
...
# Versions are pinned so this layer never calls the unauthenticated GitHub API
# (60 req/h per IP — it rate-limits during repeated rebuilds, leaving $tag empty
# and 404-ing the download). Bump these by hand when updating a tool. tarbin
# extracts a single binary from a release tarball; rawbin downloads a bare
# binary; curl -f fails the build loudly on a missing asset.
...
# KICS (Checkmarx IaC scanner) — binary + assets built in the go-builder stage.
# A wrapper injects the query/library paths so `kics scan` works from any CWD.
...
# ankamali=GDrive, berkshire=S3, choctaw=git, duroc=filesystem, essex=Confluence,
# gottingen=Jira, hante=Slack). Each release ships a separate musl zip per hog.
RUN RHVER=1.0.11 && \
...
# clone_reqs clones a repo shallowly, installs its requirements.txt when present,
# and drops the .git dir; mkwrap emits a `python3 <script>` shim on PATH. Merged
# into one layer to replace ~10 near-identical clone/pip RUNs (blackwidow's shim
# is created by the wrapper loop below).
RUN --mount=type=cache,target=/root/.cache/pip \
