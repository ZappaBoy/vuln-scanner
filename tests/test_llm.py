...
def _finding(title="SQLi", severity=Severity.HIGH) -> Finding:
    return Finding(
        ...
        description="SQL injection in login.", tool="sqlmap", target="http://target.com",
    )


def _result(findings=None) -> ScanResult:
    return ScanResult(
        tool="sqlmap", target="http://target.com",
        findings=findings or [_finding()],
        status=ScanStatus.SUCCESS, raw_output="sqlmap output here",
    )
...
class TestPocRunnerContainerGuard:
    ...
    def test_runner_refuses_without_marker(self):
        ...
        try:
            cfg = LLMConfig(
                enabled=True, api_key="sk-test", model="gpt-4o",
                features=LLMFeatures(execute_poc=True),
            )
            runner = PocRunner(cfg)
            poc = Poc(id="poc-001", language="bash", script="echo vulnerable",
                      safe_to_run=True)
            assessment = Assessment.from_results([_result()])
...
class TestAssessment:
    ...
    def test_finding_backward_compat(self):
        f = Finding(
            title="old", severity=Severity.LOW,
            description="d", tool="t", target="x",
        )
        assert f.cwe == []
